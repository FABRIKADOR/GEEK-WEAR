-- Primero, eliminar las tablas existentes con CASCADE para eliminar también las dependencias
DROP TABLE IF EXISTS order_items CASCADE;
DROP TABLE IF EXISTS orders CASCADE;
DROP TABLE IF EXISTS order_statuses CASCADE;
DROP TABLE IF EXISTS order_status_history CASCADE; -- Añadimos esta tabla que tiene dependencias
...
-- Recrear la tabla order_status_history si es necesaria
CREATE TABLE IF NOT EXISTS order_status_history (
  id UUID PRIMARY KEY,
  order_id UUID NOT NULL REFERENCES orders(id) ON DELETE CASCADE,
  status_id VARCHAR REFERENCES order_statuses(id),
  notes TEXT,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
CREATE POLICY "Los usuarios pueden ver el historial de sus propios pedidos" 
...
CREATE POLICY "Los usuarios pueden crear historial para sus propios pedidos" 
